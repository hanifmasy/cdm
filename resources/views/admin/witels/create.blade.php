...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.witel.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.witels.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="nama_witel">{{ trans('cruds.witel.fields.nama_witel') }}</label>
                <input class="form-control {{ $errors->has('nama_witel') ? 'is-invalid' : '' }}" type="text" name="nama_witel" id="nama_witel" value="{{ old('nama_witel', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('regional') ? 'is-invalid' : '' }}" name="regional_id" id="regional_id" required>
                    @foreach($regionals as $id => $regional)
                        <option value="{{ $id }}" {{ old('regional_id') == $id ? 'selected' : '' }}>{{ $regional }}</option>
                    @endforeach
                </select>
