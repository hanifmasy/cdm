...
class PerformansiKaubisController extends Controller
{
    public function index(Request $request)
    {
        ...
        abort_if(Gate::denies('performansi_kaubis_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');

        if ($request->ajax()) {

            $query = PerformansiKaubis::select(array('kaubis', 'addon', 'sto', 'witel', DB::raw('count(*) as total')))->where('psb', 1)->where('report_month', $year_now . $month_now);
            ...
            if($request->filter_addon != '') {
                $query->where('addon', 'ILIKE', '%' . $request->filter_addon . '%');
            }
            if($request->filter_sto != '') {
                $query->where('sto', 'ILIKE', '%' . $request->filter_sto . '%');
            }
            ...
            $table = Datatables::of($query->groupBy('kaubis', 'addon', 'sto', 'witel'));
        ...
        return view('admin.performansiAddon.kaubis.index', compact('witels'));
    }
}
