...
class PerformansiStoController extends Controller
{
    public function index(Request $request)
    {
        ...
        abort_if(Gate::denies('performansi_sto_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');

        if ($request->ajax()) {

            $query = PerformansiSto::select(array('sto', 'witel', 'kaubis', DB::raw('count(*) as total_sto')))->where('psb', 1);
            ...
            $table = Datatables::of($query->groupBy('sto', 'witel', 'kaubis'));
            ...
            $table->editColumn('total_sto', function ($row) {
                return $row->total_sto ? $row->total_sto : "";
            });
        ...
        return view('admin.performansiAddon.sto.index', compact('witels'));
    }
}
