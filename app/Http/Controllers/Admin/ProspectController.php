    ...
    public function index(Request $request)
    {
        if($request->ajax()){
            ...
            if (empty($request->all())) {
                ...
                $data['customers'] = MasterData::where('lis_prm', '1')->where('cprod', '11')->count();
                $data['mobiles'] = MasterData::where('lis_prm', '1')->where('cprod', '11')->where('no_hp', '!=', null)->count();
                $data['email'] = MasterData::where('lis_prm', '1')->where('cprod', '11')->where('email_myih', '!=', null)->count();
            } else {
                ...
                $query = DB::connection('pg3')->table('MASTERDATATREG6');
                $table = DB::connection('pg3')->table('MASTERDATATREG6');
                if($request->witel)
                {
                ...
                if($request->revenue)
                {
                    if(count($request->revenue) == 1)
                    {
                        if (in_array("GOLD", $request->revenue)) {
                            ...
                            $query->Where('segmen_hvc', '=', 'HVC_GOLD');                                                        ;
                            $table->Where('segmen_hvc', '=', 'HVC_GOLD');                                                        ;
                        }
                        if (in_array("PLATINUM", $request->revenue)) {
                            ...
                            $query->Where('segmen_hvc', '=', 'HVC_PLATINUM');                                                        ;
                            $table->Where('segmen_hvc', '=', 'HVC_PLATINUM');
                        }
                        if (in_array("REGULER", $request->revenue)) {
                            ...
                            $query->Where('segmen_hvc', '=', 'HVC_REGULER');                                                        ;
                            $table->Where('segmen_hvc', '=', 'HVC_REGULER');
                        }
                        if (in_array("SILVER", $request->revenue)) {
                            ...
                            $query->Where('segmen_hvc', '=', 'HVC_SILVER');                                                        ;
                            $table->Where('segmen_hvc', '=', 'HVC_SILVER');
                        }
                        if (in_array("VVIP", $request->revenue)) {
                            // $query->where('revenue_trems', '>=', '0')->where('revenue_trems', '<=', '299999')->where('usia_ps', '>', 18);;
                            // $table->where('revenue_trems', '>=', '0')->where('revenue_trems', '<=', '299999')->where('usia_ps', '>', 18);;
                            $query->Where('segmen_hvc', '=', 'HVC_VVIP');                                                        ;
                            $table->Where('segmen_hvc', '=', 'HVC_VVIP');
                        }
                    }
                    if(count($request->revenue) > 1)
                    {
                        $query->Where(function ($query) use ($request){
                            if (in_array("GOLD", $request->revenue)) {
                                ...
                                $query->orWhere('segmen_hvc','=','HVC_GOLD');
                            }
                            if (in_array("PLATINUM", $request->revenue)) {
                                // $query->orWhere('revenue_trems', '>=', '700000')->where('usia_ps', '>', 3);
                                $query->orWhere('segmen_hvc','=','HVC_PLATINUM');
                            }
                            if (in_array("REGULER", $request->revenue)) {
                                // $query->orWhere('revenue_trems', '>=', '700000')->where('usia_ps', '>', 3);
                                $query->orWhere('segmen_hvc','=','HVC_REGULER');
                            }
                            if (in_array("SILVER", $request->revenue)) {
                                // $query->orWhere('revenue_trems', '>=', '700000')->where('usia_ps', '>', 3);
                                $query->orWhere('segmen_hvc','=','HVC_SILVER');
                            }
                            if (in_array("VVIP", $request->revenue)) {
                                // $query->orWhere('revenue_trems', '>=', '700000')->where('usia_ps', '>', 3);
                                $query->orWhere('segmen_hvc','=','HVC_VVIP');
                            }
                        });
                        $table->Where(function ($table) use ($request){
                            if (in_array("GOLD", $request->revenue)) {
                                //$table->orWhere('revenue_trems', '>=', '700000')->where('usia_ps', '>', 3);
                                $table->orWhere('segmen_hvc','=','HVC_GOLD');
                            }
                            if (in_array("PLATINUM", $request->revenue)) {
                                //$table->orWhere('revenue_trems', '>=', '500000')->Where('revenue_trems', '<=', '699999')->where('usia_ps', '>', 18);
                                $table->orWhere('segmen_hvc','=','HVC_PLATINUM');
                            }
                            if (in_array("REGULER", $request->revenue)) {
                                //$table->orWhere('revenue_trems', '>=', '300000')->Where('revenue_trems', '<=', '499999')->where('usia_ps', '>', 18);
                                $table->orWhere('segmen_hvc','=','HVC_REGULER');
                            }
                            if (in_array("SILVER", $request->revenue)) {
                                //$table->orWhere('revenue_trems', '>=', '0')->Where('revenue_trems', '<=', '299999')->where('usia_ps', '>', 18);
                                $table->orWhere('segmen_hvc','=','HVC_SILVER');
                            }
                            if (in_array("VVIP", $request->revenue)) {
                                //$table->orWhere('revenue_trems', '>=', '0')->Where('revenue_trems', '<=', '299999')->where('usia_ps', '>', 18);
                                $table->orWhere('segmen_hvc','=','HVC_VVIP');
                            }
                            // if (in_array("Unbill", $request->revenue)) {
                            //     //$table->orWhere(function ($table){
                            //     //$table->orWhere('revenue_trems', '<', '0')->orWhereNull('revenue_trems');
                            //     });
                            // }
                        });
                    }
                ...
                $data = [
                    'customers' => $query->where('lis_prm', '1')->where('cprod', '11')->count(),
                    'email' => $query->where('lis_prm', '1')->where('cprod', '11')->where('email_myih', '!=', null)->count(),
                    'mobiles' => $query->where('lis_prm', '1')->where('cprod', '11')->where('no_hp', '!=', null)->count(),
                    ...
                    ))->where('lis_prm','1')->where('cprod','11')->groupBy('nama_witel')->get(),
